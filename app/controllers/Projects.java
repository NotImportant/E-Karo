...
public class Projects extends Controller {
	...
	public static Result index() {
		...
		return ok(dashboard.render(Project.findInvolving(id),
				Task.findTodoInvolving(id), User.find.byId(id)));
	}
	...
	public static Result add() {
		Project newProject = Project.create("New project", form()
				.bindFromRequest().get("group"), User.findByUsername(request()
				.username()).id);
	...
	public static Result addUser(Long project) {
		if (Secured.isMemberOf(project)) {
			Project.addMember(project,
					Integer.valueOf(form().bindFromRequest().get("user")));
			return ok();
		} else {
			return forbidden();
		}
